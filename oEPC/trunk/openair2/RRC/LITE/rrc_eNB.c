...
/*	//MessagesIds msgIds;
//	msgIds. = GTPV1U_ENB_CREATE_TUNNEL_REQ;
	//msg_to_send = itti_alloc_new_message(TASK_RRC_ENB,msgIds);
		//msg_to_send = itti_alloc_new_message(TASK_RRC_ENB,RRC_UL_DCCH);/
		//gtpv1u_eNB_send_init_udp(2152);
		//msg_to_send = itti_alloc_new_message(TASK_RRC_ENB,GTPV1U_TUNNEL_DATA_IND);
		msg_to_send = itti_alloc_new_message(TASK_RRC_ENB,UDP_INIT);
		if (msg_to_send == NULL) {
			LOG_E(RRC,"Can not create a message to initiate traffic 'n");
        		return ;
		}
		...
		//UDP_INIT(msg_to_send).port = 2152;
		UDP_INIT(msg_to_send).port = 2158;
    //LG UDP_INIT(message_p).address = "0.0.0.0"; //ANY address
		//struct in_addr addr;
		//addr.s_addr = gtpv1u_data_g.enb_ip_address_for_S1u_S12_S4_up;
		//addr.s_addr = "192.168.1.3";
		//UDP_INIT(msg_to_send).address = inet_ntoa(addr);
		UDP_INIT(msg_to_send).address = "192.168.1.4";
		LOG_D(RRC, "Tx UDP_INIT IP addr %s\n", UDP_INIT(msg_to_send).address); /* */


		//itti_send_msg_to_task(TASK_UDP,INSTANCE_DEFAULT,msg_to_send);
		//IttiMsgText mesg_text;
		//mesg_text.size = 4;
	//mesg_text.text = "test";

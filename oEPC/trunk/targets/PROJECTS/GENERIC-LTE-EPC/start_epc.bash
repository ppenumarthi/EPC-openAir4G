...
#ip route del 130.245.144.183 via 130.245.144.229 dev eth0
ip route del 130.245.144.31 via 130.245.144.229 dev eth0
ip route del 130.245.144.183 via 192.168.1.2 dev eth0:oai1
##################################
...
then
    ...
    #../configure HAVE_CHECK=true --enable-debug --enable-standalone-epc --enable-raw-socket-for-sgi  LDFLAGS=-L/usr/local/lib -- commented by OAI Team
    ../configure HAVE_CHECK=true --enable-debug --enable-standalone-epc --enable-gtp1u-in-kernel LDFLAGS=-L/usr/local/lib
    #../configure HAVE_CHECK=true --enable-debug --enable-standalone-epc --enable-raw-socket-for-sgi  LDFLAGS=-L/usr/local/lib #--hacked by Phani 
else
    cd ./$OBJ_DIR
fi
...
iptables -I PREROUTING  -t raw  -i eth0:oai1 -j LOG --log-level crit --log-ip-options --log-prefix "PREROUTING raw:"
...
#    echo "run" >> ~/.gdbinit
    ...
    valgrind --tool=callgrind --callgrind-out-file=/home/phanik/oEPC/trunk/call_epc.log $OPENAIRCN_DIR/$OBJ_DIR/OAI_EPC/oai_epc -K /tmp/itti_epc.log -c $THIS_SCRIPT_PATH/$CONFIG_FILE_EPC  2>&1 | tee /home/phanik/oEPC/trunk/stdout_epc_call.log
fi     

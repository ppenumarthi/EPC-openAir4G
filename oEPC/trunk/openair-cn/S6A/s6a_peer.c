...
void s6a_peer_connected_cb(struct peer_info *info, void *arg)
{
	...
	S6A_DEBUG("Creating message for sending S1AP \n");
        message_p = itti_alloc_new_message(TASK_S6A, ACTIVATE_MESSAGE);

	S6A_DEBUG("Allocated message for sending S1AP \n");
        itti_send_msg_to_task(TASK_S1AP, INSTANCE_DEFAULT, message_p);

	S6A_DEBUG("Message sent for S1AP \n");


        /* Inform SGI that connection to HSS is established */
	S6A_DEBUG("Creating message for sending FWIP \n");
        message_p = itti_alloc_new_message(TASK_S6A, ACTIVATE_MESSAGE);

	S6A_DEBUG("Allocated message for sending FWIP \n");
        itti_send_msg_to_task(TASK_FW_IP, INSTANCE_DEFAULT, message_p);

	S6A_DEBUG("Message sent for TASK_FW_IP \n");

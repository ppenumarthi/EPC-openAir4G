                                     ...
                                     const OctetString *res)
{
    ...
    if (emm_cause == EMM_CAUSE_SUCCESS) {
        ...
        //if ( (emm_ctx == NULL) || -- If case: NFVEPC Project
        if ( 0 ) {
         //       (memcmp(res->value, &emm_ctx->vector.xres, res->length) != 0) ) {
            /* RES does not match the XRES parameter --NFVEPC Project to print */
            LOG_TRACE(WARNING, "EMM-PROC  - Failed to authentify the UE:: res->valu: %s emm_ctx->vector.xres: %s", res->value, emm_ctx->vector.xres);

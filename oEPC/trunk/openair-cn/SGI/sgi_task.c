...
{
	read_from_socket_number(clientNum-1);
}
...
static void listen_on_server()
{
    ...
    if (socket_desc == -1)
    {
        SGI_IF_ERROR("Could not create socket\n");
    }
    ...
    {
        SGI_IF_ERROR("SOCK:%d Connection accepted from a new BS\n", client_sock);

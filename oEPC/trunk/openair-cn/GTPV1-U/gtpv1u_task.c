    ...
    while(1) {
        ...
        switch (ITTI_MSG_ID(received_message_p))
        {
            case GTPV1U_CREATE_TUNNEL_REQ: {
            	...
            	GTPU_DEBUG("-- GTPV1U_CREATE_TUNNEL_REQ ------------------------\n%d :\n", GTPV1U_CREATE_TUNNEL_REQ);
	    }
            break;
...
//#if !defined(ENABLE_USE_GTPU_IN_KERNEL)
#if !defined(ENABLE_USE_GTPU_IN_KERNEL)//--NFVEPC Project, as udp_init is happening when not in Kernel mode. Now, udp_init happens in Kernel mode

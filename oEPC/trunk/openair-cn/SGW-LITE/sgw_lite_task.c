...
/*SPGW_APP_DEBUG("-------------------------------\n" );
  SPGW_APP_DEBUG("Current local time and date: %.6lf \n", t1 );
  SPGW_APP_DEBUG("Received message as %d with size as: %d\n", ITTI_MSG_ID(received_message_p), sizeof( *received_message_p) ); */
  //SPGW_APP_DEBUG("SGW_CREATE_SESSION_REQUEST: %d, GTPV1U_CREATE_TUNNEL_RESP: %d, SGI_CREATE_ENDPOINT_RESPONSE: %d, TERMINATE_MESSAGE: %d \n", SGW_CREATE_SESSION_REQUEST, GTPV1U_CREATE_TUNNEL_RESP, SGI_CREATE_ENDPOINT_RESPONSE, TERMINATE_MESSAGE);
  if( ITTI_MSG_ID(received_message_p) == 76 )
  SPGW_APP_DEBUG("%.6lf %s %d\n", t1, "SGW_CREATE_SESSION_REQUEST", sizeof( *received_message_p) );
  if( ITTI_MSG_ID(received_message_p) == 13)
  SPGW_APP_DEBUG("%.6lf %s %d\n", t1, "GTPV1U_CREATE_TUNNEL_RESP", sizeof( *received_message_p) );
  if( ITTI_MSG_ID(received_message_p) == 21 )
  SPGW_APP_DEBUG("%.6lf %s %d\n", t1, "SGI_CREATE_ENDPOINT_RESPONSE", sizeof( *received_message_p) );
  if( ITTI_MSG_ID(received_message_p) == 3 )
  SPGW_APP_DEBUG("%.6lf %s %d\n", t1, "TERMINATE_MESSAGE", sizeof( *received_message_p) );
  SPGW_APP_DEBUG("-------------------------------\n");
/*

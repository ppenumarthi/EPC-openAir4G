...
#if defined(ENABLE_NEW_MULTICAST)
    LOG_D(EMU, "ENABLE_NEW_MULTICAST\n");
# if defined(ENABLE_PGM_TRANSPORT)
    LOG_D(EMU, "ENABLE_PGM_TRANSPORT\n");
...
# endif
    ...
    LOG_D(EMU, "Entering Switch\n");
        ...
        LOG_D(EMU, "[MAC][BYPASS] exiting switch case\n");
        num_bytesP=0;

        LOG_D(EMU, "[MAC][BYPASS] bypass_signal_mac_phy\n");
#if !defined(ENABLE_NEW_MULTICAST)
        emul_low_mutex_var=1;
    ...
    LOG_D(EMU, "[MAC][BYPASS] FINISHING\n");
    return bytes_read;
}
